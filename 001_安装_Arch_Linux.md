...
├─nvme0n1p2 259:2    0   512G  0 part /home、/var/cache、/var/log、/.snapshots、/
...
|/dev/nvme0n1p2（示例）|Linux 根分区|ext4/Btrfs|剩余空间（或按需分配）|系统根目录（/）|
|/dev/nvme0n1p3（示例）|Linux swap 分区|swap|内存 ≤ 8GB 设为 1.5 倍内存；内存 > 8GB 设为 8GB - 16GB|虚拟内存|

### 6.3 分区工具操作（fdisk）

以目标磁盘为 /dev/nvme0n1 为例，执行以下命令进入 fdisk 分区工具：
...
- d：删除分区（需输入分区号，如 d 1 表示删除第 1 个分区）

- n：创建新分区（按提示选择分区类型、分区号、起始扇区、结束扇区，建议使用默认起始扇区，结束扇区按大小计算，如 +512M 表示分区大小为 512MB）

- t：修改分区类型（EFI 分区类型代码为 1，swap 分区为 19，Linux 根分区为 20）

- w：保存分区表并退出（执行后分区生效，数据不可恢复）

- q：放弃修改并退出

操作步骤：删除目标磁盘原有分区 → 创建 EFI 分区（类型 1） → 创建 swap 分区（类型 19） → 创建根分区（类型 20） → 执行 w 保存退出。
...
# 创建根目录挂载点
mkdir -p /mnt
# 格式化根分区为 ext4（将 /dev/nvme0n1p2 替换为你的根分区路径）
...
# 创建 EFI 分区挂载点
mkdir -p /mnt/efi
# 格式化 EFI 分区为 FAT32（将 /dev/nvme0n1p1 替换为你的 EFI 分区路径）
...
# 格式化 swap 分区（将 /dev/nvme0n1p3 替换为你的 swap 分区路径）
...
# 创建根目录挂载点
mkdir -p /mnt
# 格式化根分区为 Btrfs（-f 强制格式化，将 /dev/nvme0n1p2 替换为你的根分区路径）
...
# 挂载 @log 子卷到 /mnt/var/log（日志文件不压缩，避免性能损耗）
mount -o subvol=@log,noatime,discard=async /dev/nvme0n1p2 /mnt/var/log
# 挂载 @cache 子卷到 /mnt/var/cache（缓存文件不压缩）
...
设置默认语言为中文（也可设置为 en_US.UTF-8 英文）：
